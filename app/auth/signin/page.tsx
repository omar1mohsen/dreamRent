...
function Page() {
  ...
  // open a nested modal to update password 
  ...
  // handle login funcation
  const logIn = async (e: React.FormEvent<HTMLFormElement>)=>{
    e.preventDefault()
    try{
      await signIn(email,password).then((res : any)=>{
        res.user && navigate.push('/') 
        res.user && toast.success(`Welcome Back ${res.user.displayName}`)
      })
    }catch(error){
  ...
  <div className="bg-gray-100 text-gray-500 rounded-3xl shadow-xl w-full overflow-hidden" style={{maxWidth: 1000}}>
    <div className="md:flex w-full">
      ...
      <div className="w-full md:w-1/2 py-10 px-5 md:px-10">
        ...
        <div>
        <AuthByGoogle type = 'login' />
          <form onSubmit={logIn}>
          <div className="flex -mx-3">
            <div className="w-full px-3 mb-5">
              <label  className="text-xs font-semibold px-1">Email</label>
              {emailError && (
                        <p className="text-[var(--red-text-color)] font-semibold italic text-xs  mt-1 px-1">
                          {emailError}
                        </p>
                )}
              <div className="flex">
                ...
                <input type="email" id='email' onChange={onChange} value={email} required className="w-full -ml-10 pl-2 focus:shadow-md shadow-blue-500 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="johnsmith@example.com" />
              </div>
            </div>
          </div>
          <div className="flex -mx-3">
            ...
            {passError && (
              ...
              <div className="flex">
                ...
                <input type="password" id='password' onChange={onChange} value={password} required className="w-full -ml-10 pl-2 focus:shadow-md shadow-blue-500 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="************" />
              </div>
                <span onClick={handleOpen} className=' py-1 underline block w-fit ms-auto  font-semibold text-[--sec-text-color] cursor-pointer '>forgot password ?</span>
            </div>
          </div>
          <AuthNavigate linkToSignUp={true}/>
          <div className="flex -mx-3">
            <div className="w-full px-3 mb-5">
